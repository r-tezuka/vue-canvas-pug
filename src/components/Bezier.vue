  ...
  p
    span.col-3 曲率 /curvature:
      ...
      label(for="curvature.dimension") dimension:
        input#curvature.dimension.cubic(
          type="radio",
          v-model="curvature.dimension",
          value="cubic"
        )
        label(for="curvature.dimension.cubic") cubic
        input#curvature.dimension.quadratic(
          type="radio",
          v-model="curvature.dimension",
          value="quadratic"
        )
      ...
      label(for="curvature.curveType") curveType:
        input#curvature.curveType.bezier(
          type="radio",
          v-model="curvature.curveType",
          value="bezier"
        )
        label(for="curvature.curveType.bezier") bezier
        input#curvature.curveType.rationalBezier(
          type="radio",
          v-model="curvature.curveType",
          value="rationalBezier"
        )
        label(for="curvature.curveType.rationalBezier") rationalBezier
    span.col-4 3次有理ベジェをピッチでグレーディング /grading cubic rational bezier by pitch:
      div
        label(for="cubic.numOfSize") number of sizes:
        input(type="number", v-model="cubic.numOfSize")
        input#cubic.gradingResult(
          type="checkbox",
          v-model="cubic.gradingResult"
        )
      ...
      div(v-for="(pitch, index) in this.cubic.pitches")
        div point{{ index }}
          label(for="pitch.x") &nbsp;x:
          input(type="number", v-model="pitch.x")
          label(for="pitch.y") &nbsp;y:
          input(type="number", v-model="pitch.y")
          label(for="pitch.l") &nbsp;handle length:
          input(type="number", v-model="pitch.l")
          label(for="pitch.a") &nbsp;angle:
          input(type="number", v-model="pitch.a")
          label(for="pitch.w") &nbsp;weight:
          input(type="number", v-model="pitch.w")
  canvas#canvas(
    @mousedown="onDown",
    @mouseup="onUp",
    @mouseout="onUp",
  ...
  methods: {
    ...
    draw: function() {
      ...
      if (this.quadratic.bezier || this.quadratic.rationalBezier) {
        ...
        if (this.quadratic.bezier) {
          ...
          if (
            this.curvature.circle &&
            this.curvature.dimension == 'quadratic' &&
            this.curvature.curveType == 'bezier'
          ) {
            this.drawCurvatureCircle(curve)
          }
    ...
    getQuadraticRationalBezier: function() {
      ...
      for (var i = 0; i <= 1 / this.dt; i++) {
        ...
        const x2 = this.quadratic.vertices[1].x
        const xtc =
          (1 - t) ** 2 * x0 * w0 + 2 * (1 - t) * t * x1 * w1 + t ** 2 * x2 * w2
        ...
        const y2 = this.quadratic.vertices[1].y
        const ytc =
          (1 - t) ** 2 * y0 * w0 + 2 * (1 - t) * t * y1 * w1 + t ** 2 * y2 * w2
    ...
    executeGrading: function() {
      const masterHandleRadians = [
        this.getRadian(this.cubic.vertices[0], this.cubic.handles[0]),
        this.getRadian(this.cubic.vertices[1], this.cubic.handles[1])
      ]
      const masterHandleLengths = [
        this.getLength(this.cubic.vertices[0], this.cubic.handles[0]),
        this.getLength(this.cubic.vertices[1], this.cubic.handles[1])
      ]
      for (let i = 1; i < this.cubic.numOfSize; i++) {
        ...
        for (let j in this.cubic.vertices) {
          ...
          const handle = {
            x:
              vertex.x +
              (masterHandleLengths[j] + dl) *
                Math.cos(masterHandleRadians[j] + da),
            y:
              vertex.y +
              (masterHandleLengths[j] + dl) *
                Math.sin(masterHandleRadians[j] + da),
